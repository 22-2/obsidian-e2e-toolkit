...
var PluginManager = class {
	...
	copyPluginFiles(sourcePath, destDir, pluginId) {
		...
		for (const file of (0, fs.readdirSync)(sourcePath)) {
			...
			(0, fs.copyFileSync)(srcFile, path.default.join(destDir, file));
	...
	updateCommunityPluginsJson(installedIds) {
		(0, fs.writeFileSync)(path.default.join(this.vaultPath, ".obsidian", "community-plugins.json"), JSON.stringify(installedIds));
	}
	...
	async verifyCommunityPluginsEnabled(page) {
		(0, _playwright_test.expect)(await this.isCommunityPluginsEnabled(page), "Failed to enable community plugins.").toBe(true);
	}
};
...
var ObsidianAPI = class {
	...
	async command(commandId) {
		(0, playwright_test.expect)(await this.appState((id) => {
	...
	async closeTab() {
		(0, playwright_test.expect)(await this.appState((commandId) => {
			const leaf = app.workspace.activeLeaf;
	...
	async expectContent(content) {
		(0, playwright_test.expect)(await this.content()).toBe(content);
	}
	async expectContentContains(text) {
		(0, playwright_test.expect)(await this.content()).toContain(text);
	}
	async titleBarText() {
...
async function downloadToFile(url, destPath) {
	...
	await (0, fs_promises.writeFile)(destPath, Buffer.from(await res.arrayBuffer()));
}
