...
var ObsidianAPI = class {
  ...
  async command(commandId) {
    ...
    (0, import_test.expect)(success).toBe(true);
  }
  ...
  async clickClose() {
    ...
    await (0, import_test.expect)(closeBtn).toBeVisible();
  ...
  async expectViews(viewType, count) {
    await (0, import_test.expect)(this.allViews(viewType)).toHaveCount(count);
  }
  async expectTitle(viewType, title) {
    await (0, import_test.expect)(this.title(viewType)).toHaveText(title);
  }
  async expectTitleContains(viewType, text) {
    await (0, import_test.expect)(this.title(viewType)).toContainText(text);
  }
  async expectActiveType(type) {
    await (0, import_test.expect)(this.activeTab).toHaveAttribute("data-type", type);
  }
  async expectTabs(count) {
    await (0, import_test.expect)(this.allTabs).toHaveCount(count);
  }
  async expectExists(path9) {
    (0, import_test.expect)(await this.exists(path9)).toBe(true);
  }
  async expectNotExists(path9) {
    (0, import_test.expect)(await this.exists(path9)).toBe(false);
  }
  async expectContent(content) {
    await (0, import_test.expect)(this.activeEditor).toHaveText(content);
  }
  async expectContentContains(text) {
    await (0, import_test.expect)(this.activeEditor).toContainText(text);
  }
  // ========================================
  // UI - その他のUI操作
  // ========================================
  ...
  async launchElectronApp() {
    try {
      ...
      if (!(0, import_fs3.existsSync)(distDir)) {
        ...
        logger3.error(`Electron root contents:`, await import_promises.default.readdir(electronRoot));
        throw new Error(
    ...
    return await import_test2._electron.launch(launchOptions);
  }
  async cleanup() {
    if (this.electronApp) {
      try {
...
var PluginManager = class {
  ...
  async verifyCommunityPluginsEnabled(page) {
    ...
    (0, import_test3.expect)(isEnabled, "Failed to enable community plugins.").toBe(true);
  }
};
