...
var logger3 = log3.getLogger("PluginManager");
var PluginManager = class {
  ...
  ensurePluginsDirectory() {
    const obsidianDir = path4.join(this.vaultPath, ".obsidian");
    const pluginsDir = path4.join(obsidianDir, "plugins");
    if (!existsSync3(obsidianDir)) {
  ...
  async installSingle(pluginsDir, plugin) {
    ...
    const destDir = path4.join(pluginsDir, plugin.pluginId);
    if (plugin.symlink) {
  ...
  validatePluginPath(plugin) {
    if (!existsSync3(plugin.path)) {
      console.warn(`Plugin path not found: ${plugin.path}`);
      return false;
    }
    if (!existsSync3(path4.join(plugin.path, "manifest.json"))) {
  ...
  copyPluginFiles(sourcePath, destDir, pluginId) {
    ...
    for (const file of readdirSync(sourcePath)) {
      const srcFile = path4.join(sourcePath, file);
  ...
  updateCommunityPluginsJson(installedIds) {
    const pluginsJsonPath = path4.join(
      this.vaultPath,
      ".obsidian",
...
var ObsidianAPI = class {
  ...
  async clickClose() {
    ...
    await expect2(closeBtn).toBeVisible();
  ...
  async expectViews(viewType, count) {
    await expect2(this.allViews(viewType)).toHaveCount(count);
  }
  async expectTitle(viewType, title) {
    await expect2(this.title(viewType)).toHaveText(title);
  }
  async expectTitleContains(viewType, text) {
    await expect2(this.title(viewType)).toContainText(text);
  }
  async expectActiveType(type) {
    await expect2(this.activeTab).toHaveAttribute("data-type", type);
  }
  async expectTabs(count) {
    await expect2(this.allTabs).toHaveCount(count);
  }
  ...
  async expectContent(content) {
    await expect2(this.activeEditor).toHaveText(content);
  }
  async expectContentContains(text) {
    await expect2(this.activeEditor).toContainText(text);
  }
  // ========================================
  // UI - その他のUI操作
  // ========================================
...
var logger4 = log4.getLogger("ElectronAppManager");
var ElectronAppManager = class {
  ...
  async launchElectronApp() {
    try {
      ...
      const electronRoot = path5.dirname(pkgJson);
      const distDir = path5.join(electronRoot, "dist");
      if (!existsSync4(distDir)) {
        logger4.error(`Electron dist not found at ${distDir}`);
        logger4.error(`Electron root contents:`, await fs.readdir(electronRoot));
        throw new Error(
