...
//#endregion
//#region node_modules/.pnpm/tsdown@0.22.14_typescript@6.0.3/node_modules/tsdown/esm-shims.js
...
const logger$8 = log.getLogger("constants");
/**
...
const logger$7 = log.getLogger("ElectronAppManager");
	...
	async launchElectronApp() {
		try {
			...
			if (!existsSync(distDir)) {
				...
				logger$7.error(`Electron root contents:`, await fs.readdir(electronRoot));
...
const logger$6 = log.getLogger("ObsidianCli");
/**
...
var ObsidianCli = class {
	...
	pathsMatch(expected, actual) {
		const normalizePath = (value) => {
			return path$1.normalize(path$1.resolve(value)).replace(/[\\/]$/, "");
		};
...
var PluginManager = class {
	...
	updateCommunityPluginsJson(installedIds) {
		const pluginsJsonPath = path.join(this.vaultPath, ".obsidian", "community-plugins.json");
	...
	async verifyCommunityPluginsEnabled(page) {
		...
		expect$1(isEnabled, "Failed to enable community plugins.").toBe(true);
	}
};
...
var ObsidianAPI = class {
	...
	async expectContent(content) {
		...
		expect$2(value).toBe(content);
	}
	async expectContentContains(text) {
		const value = await this.content();
		expect$2(value).toContain(text);
	}
	async titleBarText() {
